...
var DatePicker = Widget.extend({
    ...
    if (this.isInput) { // single input
      events = {
        'keyup': _.bind(function(e) {
          if (specialKeyCodes.indexOf(e.keyCode) === -1) this.update()
        }, this),
        'keydown': 'keydown',
  ...
  update: function(date) {
    ...
    if (fromArgs) {
      ...
      if (String(oldDates) !== String(this.date)) this._trigger('change')
    }
    if (!this.date && oldDates) this._trigger('clear')
    ...
    while (prevMonth.valueOf() < nextMonth) {
      if (prevMonth.getDay() === this.options.weekStart) {
        ...
        if (this.options.calendarWeeks) {
          ...
          var ws = new Date(+prevMonth + (this.options.weekStart - prevMonth.getDay() - 7) % 7 * 864e5),
